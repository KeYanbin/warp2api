...
from ..core.pool_auth import acquire_pool_or_anonymous_token, mark_quota_exhausted_and_get_new_token
...
async def send_protobuf_to_warp_api(
    ...
    try:
        ...
        timeout_config = httpx.Timeout(
            ...
            read=READ_TIMEOUT,         # 读取超时（适合长SSE流）
            write=WRITE_TIMEOUT,       # 写入超时
            pool=POOL_TIMEOUT          # 连接池超时
        )
        async with httpx.AsyncClient(http2=True, timeout=timeout_config, verify=verify_opt, trust_env=True) as client:
                ...
                async with client.stream("POST", warp_url, headers=headers, content=protobuf_bytes) as response:
                    if response.status_code != 200:
                        ...
                        if response.status_code == 429 and attempt == 0 and (
                            ...
                            logger.warning("WARP API 返回 429 (配额用尽)。标记当前账号并获取新账号…")
                            try:
                                # 标记当前账号配额用尽并获取新账号
                                new_jwt = await mark_quota_exhausted_and_get_new_token()
                            except Exception as e:
                                logger.error(f"标记配额用尽并获取新账号失败: {e}")
                                new_jwt = None
                            if new_jwt:
                                jwt = new_jwt
                                logger.info("✅ 成功获取新账号，重试请求...")
                                # 跳出当前响应并进行下一次尝试
                                continue
                            else:
                                logger.error("账号池获取失败，无法重试。")
                ...
                async with client.stream("POST", warp_url, headers=headers, content=protobuf_bytes) as response:
                    if response.status_code != 200:
                        ...
                        if response.status_code == 429 and attempt == 0 and (
                            ...
                            logger.warning("WARP API 返回 429 (配额用尽, 解析模式)。标记当前账号并获取新账号…")
                            try:
                                # 标记当前账号配额用尽并获取新账号
                                new_jwt = await mark_quota_exhausted_and_get_new_token()
                            except Exception as e:
                            ...
                            if new_jwt:
                                ...
                                logger.error("账号池获取失败，无法重试 (解析模式)。")
